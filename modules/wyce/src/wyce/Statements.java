	...
	private String createC(int opcode, Code.Convert code){
		...
		case("real"):
			if(code.type == Type.T_INT){
				r += PRE+code.target + ".type = ";
				r += "REAL_TYPE;\n";
				r += "  "+lhs_temp(9999)+"Int("+PRE+code.operand+".i);\n";
				r += "  "+PRE+code.target+".r = (double) "+PRE+"9999.i;";
				break;
			}
		case("int"):
			if(code.type == Type.T_REAL){
				...
				r += "REAL_TYPE;\n";
				r += "  "+lhs_temp(9999)+"Int("+PRE+code.operand+".i);\n";
				r += "  "+PRE+code.target+".r = (int) "+PRE+"9999.i);";
				break;
			}
			if(code.type == Type.T_CHAR){
				r += PRE+code.target + ".type = ";
				r += "INT_TYPE;\n";
				r += "  "+lhs_temp(9999)+"Char("+PRE+code.operand+".c);\n";
				r += "  "+PRE+code.target+".i = (int) "+PRE+"9999.c;";
				break;
			}
			if(code.type == Type.T_BOOL){
				r += PRE+code.target + ".type = ";
				r += "INT_TYPE;\n";
				r += "  "+lhs_temp(9999)+"Bool("+PRE+code.operand+".b);\n";
				r += "  "+PRE+code.target+".i = (int) "+PRE+"9999.b;";
				break;
			}
		case("[int]"):
...
//		}
	...
	private String lhs_temp(int c){
		return lhs_assignment_do(c, false, false);
	}
